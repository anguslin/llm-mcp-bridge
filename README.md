...
A full-stack application that connects a static frontend (GitHub Pages) to a Node.js + Express backend, which integrates with the Hugging Face LLM API and the MCP Capitol Trades server. The system processes user messages, maintains conversation history, automatically selects MCP tools via the protocol, and returns a summarized response generated by the LLM.
...
- **Backend**:
  ...
  - MCP Integration: Capitol Trades data access
...
- **Frontend**:
  ...
  - Displays LLM responses and MCP data
...
- Conversation history is stored locally per user
